/* ==========================================================================
   BKC — Catalogue data.
   Plain script (no modules) so the site runs from file:// with zero build step.
   Exposes: BKC.colors, BKC.fonts, BKC.fits, BKC.sizes, BKC.categories,
            BKC.occasions, BKC.collections, BKC.products, BKC.helpers
   ========================================================================== */
...
(function (BKC) {
  ...
  /* --- Print fonts (must match the families loaded in each page <head>) ----- */
  var FONTS = {
  ...
  var FITS = {
    oversized: {
      ...
      gsm: 240, sizes: ['S', 'M', 'L', 'XL', 'XXL', '3XL']
    },
    regular: {
      key: 'regular', label: 'Regular / Classic',
      blurb: 'Everyday 180 GSM bio-washed cotton. True to size, straight hem.',
      gsm: 180, sizes: ['XS', 'S', 'M', 'L', 'XL', 'XXL', '3XL']
    },
    crop: {
      key: 'crop', label: 'Boxy Crop',
      blurb: 'Cropped 200 GSM boxy cut. Sits at the high waist.',
      gsm: 200, sizes: ['XS', 'S', 'M', 'L', 'XL']
    },
    kids: {
      key: 'kids', label: 'Kids',
      blurb: 'Soft 160 GSM combed cotton, no-scratch neck tape, skin-safe inks.',
      gsm: 160, sizes: ['2-3Y', '4-5Y', '6-7Y', '8-9Y', '10-11Y', '12-13Y']
    }
  };
  ...
  function mk(a) {
    idn += 1;
    var color = a.c || 'white';
    var col = COLORS[color] || COLORS.white;
    var fit = a.fit || 'oversized';
    var basePrice = a.pr || (fit === 'oversized' ? 899 : fit === 'kids' ? 499 : fit === 'crop' ? 749 : 649);
    return {
      ...
      backdrop: a.bd || 'none',
      backdropHex: a.bdc || '',
      alsoIn: a.also || ['white', 'black'],
      price: basePrice,
      mrp: a.mrp || Math.round(basePrice * 2 / 50) * 50 - 1,
      audience: a.aud || ['unisex'],
      cats: a.cat || ['statement'],
      occasions: a.occ || ['everyday'],
  ...
  var PRODUCTS = [
    ...
    /* ===== Hero / brand ==================================================== */
    mk({ name: 'Bharat Ka Chootiya — The Original', p: ['BHARAT KA', 'CHOOTIYA'], f: 'anton', fit: 'oversized', c: 'white', pr: 999, mrp: 1999, aud: ['men','women','unisex'], cat: ['humour','typography','statement'], b: 'HERO DROP', r: 4.9, rev: 3128,
      d: 'The tee the whole brand is named after. Eight typefaces, one white 240 GSM canvas — pick the letterform that matches your particular brand of nonsense.' }),
    mk({ name: 'BKC Monogram', p: ['BKC'], f: 'anton', fit: 'oversized', c: 'black', pr: 899, aud: ['unisex'], cat: ['typography'], bd: 'ring', bdc: '#E7B325', b: 'NEW', r: 4.7, rev: 612,
      d: 'Three letters, one ring, zero explanation needed.' }),
    ...
    mk({ name: 'Pehle Matdaan, Fir Jalpaan', p: ['PEHLE MATDAAN', 'FIR JALPAAN'], g: '🗳️', f: 'rozha', fit: 'oversized', c: 'white', pr: 949, aud: ['men','women','unisex'], cat: ['civic','statement'], occ: ['voting'], b: 'TURNOUT DROP', r: 4.9, rev: 1640,
      d: 'Vote first, snacks after. Printed for polling day — deliberately free of any party name, colour or symbol.' }),
    mk({ name: 'Ungli Pe Nishaan', p: ['UNGLI PE', 'NISHAAN'], g: '☝️', f: 'anton', fit: 'regular', c: 'offwhite', cat: ['civic'], occ: ['voting'], r: 4.7, rev: 830,
      d: 'That indelible ink mark is the whole point. Wear the receipt.' }),
    ...
    mk({ name: 'Shanti Se, Par Zaroor', p: ['SHANTI SE,', 'PAR ZAROOR'], g: '🕊', f: 'playfair', fit: 'oversized', c: 'offwhite', cat: ['civic'], occ: ['protest'], r: 4.7, rev: 540,
      d: 'Peacefully — but definitely. A protest tee that argues for the method as much as the cause.' }),
    ...
    mk({ name: 'Bura Na Mano', p: ['BURA NA', 'MANO'], g: '🎨', f: 'marker', fit: 'oversized', c: 'white', cat: ['festive'], occ: ['holi'], b: 'HOLI', r: 4.8, rev: 1975,
      d: 'A white tee engineered to be destroyed by gulaal. Pre-shrunk so the ruin is at least the right size.' }),
    ...
    /* ===== Animals & cows ================================================== */
    mk({ name: 'Gaay Nikli Hai', p: ['GAAY', 'NIKLI HAI'], g: '🐄', f: 'marker', fit: 'oversized', c: 'white', cat: ['animals','humour'], b: 'BESTSELLER', r: 4.9, rev: 2860,
      d: 'Traffic in India has one universal override and this is it.' }),
    ...
    mk({ name: 'Cockroach Survivor', p: ['COCKROACH', 'SURVIVOR'], g: '🪳', f: 'mono', fit: 'oversized', c: 'black', cat: ['insects','humour'], r: 4.7, rev: 1188,
      d: 'Outlived the dinosaurs, will outlive your quarterly targets.' }),
    ...
    /* ===== Pride & allies =================================================== */
    mk({ name: 'Pyaar Hi Pyaar Hai', p: ['PYAAR HI', 'PYAAR HAI'], g: '🏳️‍🌈', f: 'rozha', fit: 'oversized', c: 'white', pr: 949, aud: ['pride','unisex'], cat: ['pride','statement'], b: 'PRIDE', r: 4.9, rev: 1980,
      d: 'Love is love, said in the language most of our customers argue with their families in.' }),
    ...
    mk({ name: '2018 — Struck Down', p: ['2018', 'STRUCK DOWN'], g: '⚖️', f: 'mono', fit: 'regular', c: 'indigo', aud: ['pride','unisex'], cat: ['pride','civic'], r: 4.9, rev: 1015,
      d: 'The year Section 377 was read down. A date tee, because dates are hard to argue with.' }),
  ...
  var helpers = {
    ...
    inCat: function (cat) {
      return PRODUCTS.filter(function (p) { return p.cats.indexOf(cat) !== -1; });
    },
    inOccasion: function (occ) {
      return PRODUCTS.filter(function (p) { return p.occasions.indexOf(occ) !== -1; });
    },
    withFit: function (fit) {
      return PRODUCTS.filter(function (p) { return p.fit === fit; });
    },
    forAudience: function (aud) {
      return PRODUCTS.filter(function (p) {
        return p.audience.indexOf(aud) !== -1 || p.audience.indexOf('unisex') !== -1;
      });
    },
    bestsellers: function (n) {
      return PRODUCTS.slice()
        .sort(function (a, b) { return b.reviews - a.reviews; })
        .slice(0, n || 8);
    },
    newest: function (n) {
      return PRODUCTS.slice().reverse().slice(0, n || 8);
    },
    related: function (p, n) {
      return PRODUCTS.filter(function (q) {
        if (q.id === p.id) return false;
        return q.cats.some(function (c) { return p.cats.indexOf(c) !== -1; });
      }).slice(0, n || 4);
    },
    money: function (n) {
    ...
    occLabel: function (key) {
      for (var i = 0; i < OCCASIONS.length; i++) if (OCCASIONS[i].key === key) return OCCASIONS[i].label;
      return key;
    }
  };
  ...
  BKC.fonts = FONTS;
  BKC.fits = FITS;
  BKC.categories = CATEGORIES;
  BKC.occasions = OCCASIONS;
  BKC.collections = COLLECTIONS;
  BKC.products = PRODUCTS;
  BKC.helpers = helpers;
})(window.BKC);
