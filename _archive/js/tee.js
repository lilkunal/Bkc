/* ==========================================================================
   BKC — Procedural garment renderer.
   Every product image on this site is generated SVG: no photos, no CDN.

   Goal is a real product shot, not clip art: true cut-and-sew silhouettes,
   ribbed collar, coverstitched hems, cotton weave, fold shadows, and ink that
   warps and breaks up over the knit the way a real screen print does.

   BKC.renderTee({ fit, teeHex, printHex, glyph, lines, font, detail })
   detail: 'high'  — hero / PDP (full grain + folds + warp + distress)
           'card'  — grid cards (default; folds + warp, cheaper grain)
           'flat'  — tiny thumbnails (no filters at all)
   ========================================================================== */
...
(function (BKC) {
  ...
  /* ----------------------------------------------------------------------
     Silhouettes. All share viewBox 0 0 400 470 so cards stay aligned.
     Outlines are drawn the way a tee is actually cut: shoulder slope, sleeve
     hanging to a diagonal cuff, underarm curve into the side seam, and a hem
     that dips slightly because knit fabric never lies straight.
     ---------------------------------------------------------------------- */
  var SHAPES = {
    /* Sleeve is a real quadrilateral: shoulder tip → outer seam → cuff edge →
       underarm → side seam. That cuff line is what stops it reading as an icon. */
    regular: {
      body:
    ...
    oversized: {
      /* Drop shoulder: wide low armhole, boxy body. Chest-to-length ~0.69,
         which is what an actual 240 GSM oversized measures flat. */
      body:
  ...
  /* ----------------------------------------------------------------------
     Shared <defs>. Injected into the document exactly once; every tee on the
     page references these by id, so the browser parses the filters a single
     time no matter how many garments are on screen.
     ---------------------------------------------------------------------- */
  var DEFS_ID = 'bkc-tee-defs';
  ...
  function defsMarkup() {
    ...
    '<svg id="' + DEFS_ID + '" aria-hidden="true" focusable="false" ' +
      ...
      /* Cross-body shading: light falls from upper-left, edges roll away. */
      ...
      /* Weight of the fabric pulling shadow into the hem. */
      ...
      /* Occlusion in the neck well — tight under the collar only. */
      ...
      /* Knit weave — a real ribbed jersey grid, cheap enough for 100+ cards. */
      ...
      /* Ink riding over the knit — the single biggest "this is real" cue. */
      ...
      /* Ink dropout: micro-holes where the screen didn't lay down flat. */
      ...
      /* Garment lifted off the backdrop. */
  ...
  function ensureDefs() {
    ...
    if (document.getElementById(DEFS_ID)) { defsReady = true; return; }
  ...
  function fitSize(text, maxWidth, cap) {
  ...
  /* Shade a hex by a factor — used for collar rib and seam thread. */
   ...
   */
  BKC.renderTee = function (o) {
    o = o || {};
    ensureDefs();

    var shape = SHAPES[o.fit] || SHAPES.regular;
    var tee = o.teeHex || '#FFFFFF';
    var ink = o.printHex || '#0E0E0C';
    var font = o.font || "'Anton', Impact, sans-serif";
    var lines = (o.lines || []).filter(Boolean).slice(0, 3);
    var detail = o.detail || (o.flat ? 'flat' : 'card');
    var flat = detail === 'flat';
    var high = detail === 'high';
    var uid = 'r' + Math.random().toString(36).slice(2, 8);
    var pr = shape.print;
    var light = luma(tee) > 150;

    /* Seam thread reads darker on pale cloth, lighter on dark cloth. */
    var thread = light ? 'rgba(0,0,0,.26)' : 'rgba(255,255,255,.30)';
    var edge = light ? 'rgba(0,0,0,.42)' : 'rgba(0,0,0,.55)';

    /* ---- backdrop shape behind the print ---- */
    var backdrop = '';
    if (o.backdrop && BACKDROPS[o.backdrop] && o.backdrop !== 'none') {
      backdrop = BACKDROPS[o.backdrop]
        .replace(/\{c\}/g, o.backdropHex || ink)
        .replace(/\{cy\}/g, pr.y)
        .replace(/\{by\}/g, pr.y - 84)
        .replace(/\{ry\}/g, pr.y - 27)
        .replace(/\{sy\}/g, pr.y - 118);
    }

    /* ---- print block, centred on the chest anchor ---- */
    var glyphSize = lines.length ? 74 : 116;
    var hasGlyph = !!o.glyph;
    var cap = pr.w > 160 ? 44 : 38;
    var sizes = lines.map(function (l, i) { return fitSize(l, pr.w, i === 0 ? cap : cap - 6); });
    var totalH = sizes.reduce(function (a, s) { return a + s * 1.06; }, 0) + (hasGlyph ? glyphSize * 0.92 : 0);
    var cursor = pr.y - totalH / 2;

    var print = '';
    if (hasGlyph) {
      cursor += glyphSize * 0.78;
      print += '<text x="' + pr.x + '" y="' + cursor.toFixed(1) + '" font-size="' + glyphSize +
        '" text-anchor="middle">' + esc(o.glyph) + '</text>';
      cursor += glyphSize * 0.2;
    }
    lines.forEach(function (l, i) {
      cursor += sizes[i] * 0.96;
      print += '<text x="' + pr.x + '" y="' + cursor.toFixed(1) + '" font-size="' + sizes[i].toFixed(1) +
        '" font-family="' + esc(font) + '" fill="' + esc(ink) +
        '" text-anchor="middle" letter-spacing="0.5">' + esc(l) + '</text>';
      cursor += sizes[i] * 0.12;
    });

    /* Ink is warped by the knit, broken up by dropout, and multiplied so the
       weave underneath still shows through. Slight rotation = hand-placed. */
    var printGroup = '';
    if (backdrop || print) {
      var inkAttrs = flat ? '' :
        ' filter="url(#bkcInkWarp)" mask="url(#bkcInkMask)" style="mix-blend-mode:multiply"';
      printGroup =
        '<g transform="rotate(-0.5 200 ' + pr.y + ')" opacity="' + (flat ? 1 : 0.93) + '"' + inkAttrs + '>' +
          backdrop + print +
        '</g>';
    }

    /* ---- fabric behaviour: folds, weave, shading ---- */
    var folds = '', lights = '', weave = '', grain = '';
    if (!flat) {
      /* Folds vary in weight — uniform strokes read as banding, not cloth. */
      folds = '<g filter="url(#bkcSoft)" opacity=".26">' +
        shape.folds.map(function (d, i) {
          return '<path d="' + d + '" fill="none" stroke="#000" stroke-width="' +
            (i % 2 ? 6 : 10) + '" stroke-linecap="round" opacity="' + (i % 3 ? 0.8 : 0.5) + '"/>';
        }).join('') + '</g>';
      lights = '<g filter="url(#bkcSofter)" opacity=".2">' +
        shape.lights.map(function (d) {
          return '<path d="' + d + '" fill="none" stroke="#fff" stroke-width="20" stroke-linecap="round"/>';
        }).join('') + '</g>';
      weave = '<rect width="400" height="470" fill="url(#bkcWeave)"/>';
      if (high) {
        grain = '<rect width="400" height="470" filter="url(#bkcGrain)" opacity=".13" ' +
          'style="mix-blend-mode:multiply"/>';
      }
    }

    /* ---- construction details: rib collar, coverstitch, cuff seams ---- */
    var construction = '';
    if (!flat) {
      construction =
        /* Ribbed collar band, sitting slightly proud of the body. */
        '<path d="' + shape.collar + '" fill="' + shade(tee, light ? 0.93 : 1.16) + '"/>' +
        '<path d="' + shape.collar + '" fill="url(#bkcWeave)" opacity=".8"/>' +
        '<path d="' + shape.neckLine + '" fill="none" stroke="' + thread +
          '" stroke-width="1.1" stroke-dasharray="2.5 2.5"/>' +
        /* Coverstitch: two parallel rows, exactly like a real hem. */
        '<path d="' + shape.hem + '" fill="none" stroke="' + thread +
          '" stroke-width="1" stroke-dasharray="3 3" transform="translate(0,-9)"/>' +
        '<path d="' + shape.hem + '" fill="none" stroke="' + thread +
          '" stroke-width="1" stroke-dasharray="3 3" transform="translate(0,-5)"/>' +
        /* Sleeve cuff hems — folded band plus its stitch row. */
        shape.cuffs.map(function (d) {
          return '<path d="' + d + '" fill="none" stroke="' + thread +
            '" stroke-width="1" stroke-dasharray="3 3"/>';
        }).join('') +
        /* Armhole seams where the sleeve is set into the body. */
        (shape.armholes || []).map(function (d) {
          return '<path d="' + d + '" fill="none" stroke="' + thread +
            '" stroke-width="1" stroke-dasharray="3 2.5"/>';
        }).join('') +
        /* Shading in the armhole hollow, so sleeves sit behind the chest. */
        (shape.armholes || []).map(function (d) {
          return '<path d="' + d + '" fill="none" stroke="#000" stroke-opacity=".16" ' +
            'stroke-width="9" filter="url(#bkcSoft)"/>';
        }).join('');
    }

    var ao = shape.ao || [200, 110, 88, 32];
    var shading = flat ? '' :
      '<rect width="400" height="470" fill="url(#bkcSide)"/>' +
      '<rect width="400" height="470" fill="url(#bkcVert)"/>' +
      '<ellipse cx="' + ao[0] + '" cy="' + ao[1] + '" rx="' + ao[2] + '" ry="' + ao[3] +
        '" fill="url(#bkcNeckAO)"/>';

    var cast = flat ? '' : ' filter="url(#bkcCast)"';

    return '' +
      '<svg class="tee-svg" viewBox="0 0 400 470" xmlns="http://www.w3.org/2000/svg" role="img" ' +
        'aria-label="' + esc(o.alt || 'T-shirt') + '">' +
        '<defs><clipPath id="' + uid + '"><path d="' + shape.body + '"/></clipPath></defs>' +
        '<g' + cast + '>' +
          '<path d="' + shape.body + '" fill="' + esc(tee) + '"/>' +
          '<g clip-path="url(#' + uid + ')">' +
            weave +
            printGroup +
            folds +
            ...
            construction +
          '</g>' +
          '<path d="' + shape.body + '" fill="none" stroke="' + edge + '" stroke-width="1.6" ' +
  ...
  BKC.renderProductTee = function (p, overrides) {
    var o = overrides || {};
    return BKC.renderTee({
      fit: o.fit || p.fit,
  ...
  BKC.teeFits = Object.keys(SHAPES);
  BKC.teeBackdrops = Object.keys(BACKDROPS);
})(window.BKC);
